"""Launch the multi-drone circle flight alongside the Crazyswarm2 server.

Outputs land in <repo>/results/flights/:
  circle_all_<timestamp>/        rosbag (Vicon /poses, /cfN/pose, /tf, /rosout)
  circle_all_<timestamp>.log     tee of every launched process's stdout/stderr
"""
import datetime
import os
...
def generate_launch_description():
    ...
    bag_path = os.path.join(BAG_ROOT, f'circle_all_{stamp}')
    ...
    circle_all_node = Node(
        package='particle_life',
        executable='circle_all',
        name='circle_all',
        output='screen',
    )
